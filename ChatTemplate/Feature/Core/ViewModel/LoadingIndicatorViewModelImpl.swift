...
extension LoadingIndicatorPresentableView where Self: UIViewController {
    func bindToLoadingIndicator() {
        loadingIndicatorViewModel.showProgressHUD.observeOn(MainScheduler.instance).subscribe(onNext: {[weak self] (show: Bool) in
            if show == true {
        ...
        loadingIndicatorViewModel.showIndicator.observeOn(MainScheduler.instance).subscribe(onNext: {[weak self] (show: Bool) in
            if show == true {
