...
class SwitchNavigationController: UIViewController {
    ...
    /**
     ...
     Manually trigger callback because callbacks are not automatically called when add as child view controlle and the container has already loaded view
     */
    func showViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if let rootViewController = self.rootViewController {
            ...
            if animated == true {
                UIView.animate(withDuration: 0.55, delay: 0, options: [.transitionCrossDissolve, .curveEaseOut], animations: {
