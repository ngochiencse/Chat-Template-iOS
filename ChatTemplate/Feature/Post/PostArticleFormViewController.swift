...
class PostArticleFormViewController: FormViewController, BasicViewPresentableView {
    ...
    private func bindToViewModel() {
        binding = Observable.combineLatest(viewModel.title.asObservable(), viewModel.image.asObservable(), viewModel.content.asObservable()).subscribe { (_) in
            self.tableView.reloadData()
        }
        binding?.disposed(by: rx.disposeBag)
    }
